...
BASE_NAME = "IV_sweep_+20V_to_-2V_0.5Vstep_1.0s_hold_CZT"
# ------------------------------------------------
...
ax_iv.set_title("Live I–V Sweep (Keithley 6487)")
...
ax_it.set_title("Live I–t (Keithley 6487)")
...
# Optional: reduce plot update rate to keep it snappy
PLOT_EVERY_N = 1  # set to 2 or 5 if you want fewer refreshes
...
try:
    ...
    with open(csv_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["Set Voltage (V)", "Current (A)", "Status", "t (s)", "Raw READ?"])
        ...
        for i, V in enumerate(voltages, start=1):
            ...
            writer.writerow([f"{V:.6f}", f"{curr:.12e}", f"{stat:.0f}", f"{t:.6f}", raw])

            # Console print (more decimals so you don't see duplicates)
            print(f"Vset={V:+.2f} V | I={curr:+.3e} A | STAT={stat:.0f} | t={t:7.2f} s")

            # Live plot update (throttle if needed)
            if (i % PLOT_EVERY_N) == 0:
                # I-V
